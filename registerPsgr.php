...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $stmtPassenger = $connMe->prepare("SELECT PsgrID FROM PASSENGER WHERE UserID = ?");
    $stmtPassenger->bind_param("s", $userID);
    $stmtPassenger->execute();
    $resultPassenger = $stmtPassenger->get_result();

    if ($resultPassenger->num_rows === 1) {
...
<body>
    ...
    //Email and phone checking whether new user, existing user
    emailNPhoneForm.addEventListener("submit", (e) => {
        ...
        .then((data) => {
            ...
            if (data.status === "exists_passenger") {
                ...
                checkError.textContent = "You are already registered as a passenger.";
